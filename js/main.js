 ...
 *description
 *捡元宝游戏
 *游戏主要包括五个模式：限时20s，元宝不要丢，坚持百分百，不要碰炸弹，1min大杂烩；
 *-限时20s：共有20s时间，只掉落元宝，游戏结果取元宝数
 *-元宝不要丢：共有三次元宝掉落机会，只掉落元宝，游戏结果取元宝数
 *-坚持百分百：其实生命值50，吃到一个元宝+1，吃到一个炸弹-1，分数为100或0停止，游戏结果取时间
 *-不要碰炸弹：只掉落炸弹，每次掉落1-3个，有三次被砸中机会，游戏结果取躲过的炸弹组数
 *-1min大杂烩：1min限时，同时掉落元宝（1分），钻石（2分），炸弹（-1分），游戏结果取总分数
 *游戏难度增加依据元素速度的增加
 *开发模式为phaser+bootstrap
 *记录查询部分使用localstorage，排行榜部分使用ajax
 *游戏平台需要是移动端，控制方式为触控
 *支持屏幕横竖检测，只能在竖屏条件下运行
 *可设置游戏音效开关
 *游戏各模式程序置于gameArr数组中
 *email     1148586347@qq.com
 *@date     2015-7-25
 *@author   光光
**/
//分数数组
...
gameArr[0] = {
	...
	create : function(){
		this.dropTime = 300;
		this.player = this.game.add.sprite(0,gameHeight - 80,"player");
